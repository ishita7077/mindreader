# MindReader — Folders, Repos, and Source of Truth
...
| `Brain Diff/` | Documentation snapshot (not git; legacy local folder name) | — | — | — | Loose `.md` and `.html` files: `BUILDER_LOG.md`, `CURSOR.md`, `a11y_review.md`, `handoff_results.md`, plus standalone `landing.html`, `results.html`, `run.html`. Treat as historical scratch. |
| `braindiff_v1-main/` | Source-of-truth specs (not git) | — | — | — | Original spec set: `DIMENSIONS_SPEC_FINAL.md`, `IMPLEMENTATION_PLAN_FINAL.md`, `NORTH_STAR_FINAL.md`, `RUN_LOCAL.md`, `UX_SPEC_FINAL.md`, plus the legacy audit workbook. Reference only — code in `backend/`, `atlases/` here is a frozen snapshot from earlier. |
...
|---|---|
| **`https://github.com/ishita7077/mindreader`** | **Only push target** for the active folder. Local `origin` points here. **`main`** is what Vercel/GitHub Actions should track for deploy. |
...
- **`git remote -v`** should show a single **`origin`** → `https://github.com/ishita7077/mindreader.git`.
...
3. **`origin` remote.** Keep it on **`mindreader`** for this deploy path unless you deliberately add a second remote.
